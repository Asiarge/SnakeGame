...
public class MySnake {
    ...
    private ArrayList<Snake> arrSnake = new ArrayList<>();
    private int length;
    private boolean move_left, move_right, move_up, move_down;

    public MySnake(Bitmap bm, int x, int y, int length) {
        this.bm = bm;
        this.length = length;
        ...
        setMove_right(true);
        arrSnake.add(new Snake(bm_head_right, x, y));
        for (int i = 1; i < length-1; i++){
            this.arrSnake.add(new Snake(bm_body_horizontal, this.arrSnake.get(i-1).getX()- MapMenu.sizeElementMap, y));
        }
        arrSnake.add(new Snake(bm_tail_right, arrSnake.get(length-2).getX()- MapMenu.sizeElementMap, arrSnake.get(length-2).getY()));
    }
    ...
    public void update(){
        ...
        if(move_right){
            arrSnake.get(0).setX(arrSnake.get(0).getX()+ MapMenu.sizeElementMap);
            arrSnake.get(0).setBm(bm_head_right);
        }else if(move_down){
            arrSnake.get(0).setY(arrSnake.get(0).getY()+ MapMenu.sizeElementMap);
            arrSnake.get(0).setBm(bm_head_down);
        }else if(move_up){
            arrSnake.get(0).setY(arrSnake.get(0).getY()- MapMenu.sizeElementMap);
            arrSnake.get(0).setBm(bm_head_up);
        }else{
            arrSnake.get(0).setX(arrSnake.get(0).getX()- MapMenu.sizeElementMap);
            arrSnake.get(0).setBm(bm_head_left);
        }
        for (int i = 1; i < length - 1; i++){
            if(arrSnake.get(i).getrLeft().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrBottom().intersect(arrSnake.get(i-1).getrBody())
                    || arrSnake.get(i).getrBottom().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrLeft().intersect(arrSnake.get(i-1).getrBody())){
                arrSnake.get(i).setBm(bm_body_bottom_left);
            }else if (arrSnake.get(i).getrLeft().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrTop().intersect(arrSnake.get(i-1).getrBody())
                    || arrSnake.get(i).getrLeft().intersect(arrSnake.get(i-1).getrBody())
                    && arrSnake.get(i).getrTop().intersect(arrSnake.get(i+1).getrBody())){
                arrSnake.get(i).setBm(bm_body_top_left);
            }else if (arrSnake.get(i).getrRight().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrTop().intersect(arrSnake.get(i-1).getrBody())
                    || arrSnake.get(i).getrRight().intersect(arrSnake.get(i-1).getrBody())
                    && arrSnake.get(i).getrTop().intersect(arrSnake.get(i+1).getrBody())) {
                arrSnake.get(i).setBm(bm_body_top_right);
            }else if(arrSnake.get(i).getrRight().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrBottom().intersect(arrSnake.get(i-1).getrBody())
                    || arrSnake.get(i).getrRight().intersect(arrSnake.get(i-1).getrBody())
                    && arrSnake.get(i).getrBottom().intersect(arrSnake.get(i+1).getrBody())){
                arrSnake.get(i).setBm(bm_body_bottom_right);
            }else if(arrSnake.get(i).getrLeft().intersect(arrSnake.get(i-1).getrBody())
                    && arrSnake.get(i).getrRight().intersect(arrSnake.get(i+1).getrBody())
                    || arrSnake.get(i).getrLeft().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrRight().intersect(arrSnake.get(i-1).getrBody())){
                arrSnake.get(i).setBm(bm_body_horizontal);
            }else if(arrSnake.get(i).getrTop().intersect(arrSnake.get(i-1).getrBody())
                    && arrSnake.get(i).getrBottom().intersect(arrSnake.get(i+1).getrBody())
                    || arrSnake.get(i).getrTop().intersect(arrSnake.get(i+1).getrBody())
                    && arrSnake.get(i).getrBottom().intersect(arrSnake.get(i-1).getrBody())){
                arrSnake.get(i).setBm(bm_body_vertical);
            }else{
                if(move_right){
                    arrSnake.get(i).setBm(bm_body_horizontal);
                }else if(move_down){
        ...
        if(arrSnake.get(length-1).getrRight().intersect(arrSnake.get(length-2).getrBody())){
            arrSnake.get(length-1).setBm(bm_tail_right);
        }else if(arrSnake.get(length-1).getrLeft().intersect(arrSnake.get(length-2).getrBody())){
            arrSnake.get(length-1).setBm(bm_tail_left);
        }else if(arrSnake.get(length-1).getrBottom().intersect(arrSnake.get(length-2).getrBody())){
            arrSnake.get(length-1).setBm(bm_tail_down);
        }else{
            arrSnake.get(length-1).setBm(bm_tail_up);
        }
    }

    public void drawSnake(Canvas canvas){
        for(int i = length-1; i >= 0; i--){
            canvas.drawBitmap(arrSnake.get(i).getBm(), arrSnake.get(i).getX(), arrSnake.get(i).getY(), null);
        }
    }
    ...
    public ArrayList<Snake> getArrPartSnake() {
        return arrSnake;
    }

    public void setArrPartSnake(ArrayList<Snake> arrSnake) {
    ...
    public void addPart() {
        Snake p = this.arrSnake.get(length-1);
        this.length += 1;
        if(p.getBm()==bm_tail_right){
            this.arrSnake.add(new Snake(bm_tail_right, p.getX()- MapMenu.sizeElementMap, p.getY()));
        }else if(p.getBm()==bm_tail_left){
            this.arrSnake.add(new Snake(bm_tail_left, p.getX()+ MapMenu.sizeElementMap, p.getY()));
        }else if(p.getBm()==bm_tail_up){
            this.arrSnake.add(new Snake(bm_tail_up, p.getX(), p.getY()+ MapMenu.sizeElementMap));
